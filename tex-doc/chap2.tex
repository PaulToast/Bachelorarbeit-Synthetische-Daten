...
Die Erfahrung $E$ besteht dabei aus einer Menge von Trainingsdaten, die etwa aus Eingabe-Ausgabe-Paaren bestehen können, also die erwünschte Ausgabe des Programms für jede Eingabe mitliefern. Die Aufgaben $T$ können sehr unterschiedlich sein, von einfachen Klassifikations- und Regressionsaufgaben bis hin zu komplexen Problemen wie Spracherkennung oder autonomes Fahren. Das Leistungsmaß $P$ gibt an, wie gut das Programm die Aufgaben $T$ löst, und kann z.B. der Anteil korrekt klassifizierter Beispiele sein (Accuracy).

Durch das Lernen aus den Trainingsdaten wird das Programm zu einem \textit{Modell} des zugrundeliegenden Problems, das dann auf neue, unbekannte Daten angewendet werden kann, um Vorhersagen zu treffen oder Entscheidungen zu treffen.
